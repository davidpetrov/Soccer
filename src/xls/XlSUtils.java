...
public class XlSUtils {
	...
	public static Settings runForLeagueWithOdds(HSSFSheet sheet, ArrayList<Fixture> all, float minOdds)
		...
		for (int x = 0; x <= 20; x++) {
			...
			Settings set = new Settings(sheet.getSheetName(), bestxprofit * 0.05f, 1.0f - bestxprofit * 0.05f, 0.0f,
					0.55f, 0.55f, 0.55f, 1, 10, bestWinPercent, bestProfit);
		...
		return new Settings(sheet.getSheetName(), bestxprofit * 0.05f, 1.0f - bestxprofit * 0.05f, 0.0f, 0.55f, 0.55f,
	...
	public static float realisticRun(HSSFSheet sheet, int year) throws IOException {
		...
		for (int i = 11; i < maxMatchDay; i++) {
			...
			// System.out.println("Profit for " + sheet.getSheetName() + "
			// matchday " + i + " is: " + profit);
